...
def make_dataset(zipcodes_to_plot, num_neighbor, period_str):
    with open('nyc-zip-code-tabulation-areas-2012.geojson','rt') as json_file:
        nyc_json = json.load(json_file)
    nyc_json['features'][zipcodes[zipcodes_to_plot]]['properties']['color_type'] = 2

#         print(zipcodes_to_plot)

    nyc_zipcode_to_dist = pd.read_pickle('nyc_zipcode_to_dist_'+ period_str)

    for zipcode in zipcodes:
        if zipcode in nyc_zipcode_to_dist:
            nyc_json['features'][zipcodes[zipcode]]['properties']['has_data'] = 'YES'
        else:
            nyc_json['features'][zipcodes[zipcode]]['properties']['has_data'] = 'NO'


    if zipcodes_to_plot in nyc_zipcode_to_dist:
        distances = nyc_zipcode_to_dist[zipcodes_to_plot]
#         print (distances)
        num_neighbor_count = 0
        for neighbor_idx in range(len(distances)):
            if distances[neighbor_idx][0] in zipcodes:
                num_neighbor_count +=1
                nyc_json['features'][zipcodes[distances[neighbor_idx][0]]]['properties']['color_type'] = 1
            if num_neighbor_count == num_neighbor:
                break
#             for zipcode_near in distances[:num_neighbor]:
#                 if zipcode_near[0] in zipcodes:
#                     nyc_json['features'][zipcodes[zipcode_near[0]]]['properties']['color_type'] = 1
#             print (nyc_json['features'][zipcodes[zipcode_near[0]]])

#         for zip_idx in range(len(nyc_json['features'])):
#             if 'color_type' not in nyc_json['features'][zip_idx]['properties']:
#                 nyc_json['features'][zip_idx]['properties']['color_type'] = -1

#         with open('nyc_json_update.json', 'w') as fp:
#             json.dump(nyc_json, fp)
#         with open(r'nyc_json_update.json','r') as f:
#             geo_source = GeoJSONDataSource(geojson = f.read())

    # convert json to string
    geojson = json.dumps(nyc_json)
...
#         with open('nyc_json_update.json','rt') as json_file:
#             nyc_new = json.load(json_file)
...
#                   line_color='white', line_width=0.5,
    ...
    hover = HoverTool(tooltips=[('Zip Code', '@postalCode'),
                                ('Has Data', '@has_data')
                    ])
    p.add_tools(hover)
...
for zip_idx in range(len(nyc_json['features'])):
    zipcodes[nyc_json['features'][zip_idx]['properties']['postalCode']] = zip_idx
#         zipcodes_sorted.append(nyc_json['features'][zip_idx]['properties']['ZIP'])
...
nyc_zipcode_to_dist = pd.read_pickle('nyc_zipcode_to_dist_'+ 'midday')
for zipcode in nyc_zipcode_to_dist:
